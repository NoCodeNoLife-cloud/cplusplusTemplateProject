...
TEST_F(CountMinSketchTest, Add_ZeroCount_NoEffect)
{
    CountMinSketch<1024, 3> cms;
    cms.add("item_a", 5);
    cms.add("item_b", 0);  // zero count — should be a no-op
...
TEST_F(CountMinSketchTest, SmallWidth_CollisionsObservable)
{
    ...
    // Add many distinct int items to a very small sketch — collisions are
    ...
    // a tiny sketch — verify the totalCount accounts for all inserts
 ...
 *          This test documents that limitation — the user is responsible for
 *          ensuring counts stay well below uint32_t max.
 */
TEST_F(CountMinSketchTest, Add_LargeCount_NoCrash)
{
    CountMinSketch<1024, 3> cms;
    // Add a very large count — should not throw or crash
