/**
 * @file LSMTree.hpp
 * @brief Log-Structured Merge Tree (LSM-Tree) — write-optimised KV store
 ...
 *
 * tree.put(42, "hello");
 * auto v = tree.get(42);      // → std::optional{"hello"}
 * tree.remove(42);
 * auto v2 = tree.get(42);     // → std::nullopt
 * @endcode
 */

#pragma once

#include "LSMTreeOptions.hpp"
#include "SSTable.hpp"
...
// -----------------------------------------------------------------------
//  Logging helpers — disabled by default; define LSMTREE_ENABLE_LOG to
//  enable trace output.
// -----------------------------------------------------------------------
...
namespace common::data_structure::tree::lsm
{
    ...
    public:
        ...
        [[nodiscard]] auto get(const Key& key) const -> std::optional<Value>
        {
            ...
            // 3. Levels (L0 … L_max) — stop at the first match
            for (std::size_t level = 0; level < levels_.size(); ++level)
            {
                ...
                // NotFound → continue to next level
            }
        ...
        void remove(const Key& key)
        {
            ...
            if (it != memtable_.end())
            {
                if (!it->second.tombstone)
                {
                    // Replacing a live entry with a tombstone → one fewer
    ...
    private:
        ...
        /// @brief Active MemTable — an in-memory sorted map.
        ...
        // ──────────────────────────────────────────────────────────────
        //  Internal helpers — put / seal / flush / compact / search
        // ──────────────────────────────────────────────────────────────
        ...
        void internalPut(const Key& key, InternalValue iv)
        {
            ...
            if (!inserted)
            {
                // Key exists — only treat as a new live entry if the
        ...
        [[nodiscard]] auto shouldCompactLevel(std::size_t level) const -> bool
        {
            ...
            // Level i target = memtable_size × fanOut^i
        ...
        /// the merge input are tombstones — in that case the tombstone is
        ...
        [[nodiscard]] auto mergeTables(std::vector<SSTablePtr>& inputs) -> std::vector<SSTablePtr>
        {
            ...
            // non-tombstone value for the same key in the merge input — if
            ...
            // time — but that is expensive.  Instead, we apply a simple
            // conservative rule:
            //
            ...
            while (true)
            {
                ...
                // Winning entry is a tombstone → check if there's a value
                ...
                if (next->tombstone)
                {
                    ...
                    // No value found → keep the tombstone.
                }
        ...
        [[nodiscard]] auto searchLevel(const Key& key, std::size_t level) const -> LevelResult
        {
            ...
            if (level == 0)
            {
                // L0: overlapping — search newest table first.
            ...
            {
                // L1+: non-overlapping — find the single overlapping table
                // via binary search.
