...
namespace common::toolkit
{
    ...
    public:
        ...
        template <typename T>
        [[nodiscard]] static bool isValidEnumName(const std::string& name) ;

        /// @brief Get the count of enum values.
        /// @tparam T The enum type.
        /// @return size_t The number of enum values.
        template <typename T>
        [[nodiscard]] static size_t getEnumCount() ;
        ...
        template <typename T>
        [[nodiscard]] static std::underlying_type_t<T> enumToInteger(T value) ;
        ...
        template <typename T>
        [[nodiscard]] static std::optional<T> integerToEnum(std::underlying_type_t<T> value) ;

        /// @brief Get formatted information about an enum type.
        /// @tparam T The enum type.
        /// @return std::string Formatted string with enum type info and all values.
        template <typename T>
        [[nodiscard]] static std::string formatEnumInfo() ;
    };

    template <typename T>
    std::string EnumToolkit::getEnumName(T value)
    {
        return static_cast<std::string>(magic_enum::enum_name(value));
    }

    template <typename T>
    std::string EnumToolkit::getEnumTypeName()
    {
        return static_cast<std::string>(magic_enum::enum_type_name<T>());
    }

    template <typename T>
    std::optional<T> EnumToolkit::stringToEnum(const std::string& name)
    {
        return magic_enum::enum_cast<T>(name);
    }

    template <typename T>
    std::vector<T> EnumToolkit::getAllEnumValues()
    {
    ...
    template <typename T>
    std::vector<std::string> EnumToolkit::getAllEnumNames()
    {
    ...
    template <typename T>
    bool EnumToolkit::isValidEnumName(const std::string& name)
    {
        return magic_enum::enum_cast<T>(name).has_value();
    }

    template <typename T>
    size_t EnumToolkit::getEnumCount()
    {
        return magic_enum::enum_count<T>();
    }

    template <typename T>
    std::underlying_type_t<T> EnumToolkit::enumToInteger(T value)
    {
        return magic_enum::enum_integer(value);
    }

    template <typename T>
    std::optional<T> EnumToolkit::integerToEnum(std::underlying_type_t<T> value)
    {
        return magic_enum::enum_cast<T>(value);
    }

    template <typename T>
    std::string EnumToolkit::formatEnumInfo()
    {
